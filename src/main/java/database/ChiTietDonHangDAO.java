	...
	public ArrayList<ChiTietDonHang> selectAll() {
		...
		try {
			...
			while (rs.next()) {
				...
				temp.setMaDonHang(donhang);//dua vao ma don hang lay duoc de tao ra doi tuong DonHang roi tu do tiep tuc lay cac doi tuong con
				DonHang dh = new DonHangDAO().selectById(temp);
				
				SanPham tempSanPham = new SanPham();
				tempSanPham.setMaSanPham(sanpham); //ma san pham
				SanPham sp = new SanPhamDAO().selectById(tempSanPham);
	...
	public ChiTietDonHang selectById(ChiTietDonHang t) {
		...
		try {
			...
			while (rs.next()) {
				...
				DonHang temp = new DonHang();
				temp.setMaDonHang(donhang);
				DonHang dh = new DonHangDAO().selectById(temp);
				
				SanPham tempSanPham = new SanPham();
				tempSanPham.setMaSanPham(sanpham);
				SanPham sp = new SanPhamDAO().selectById(tempSanPham);

				ketQua = new ChiTietDonHang(maChiTietDonHang, dh, sp, soluong, giagoc, giamgia, giaban,
						thuevat, tongtien);
				break;
			}
			// Bước 5:
